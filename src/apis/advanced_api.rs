...
pub async fn advanced(configuration: &configuration::Configuration, params: AdvancedParams) -> Result<models::Advanced200Response, Error<AdvancedError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Advanced200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Advanced200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_api_usage(configuration: &configuration::Configuration, params: GetApiUsageParams) -> Result<models::GetApiUsage200Response, Error<GetApiUsageError>> {
    // Extract parameters from params struct
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetApiUsage200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetApiUsage200Response`")))),
        }
    } else {
        let content = resp.text().await?;
